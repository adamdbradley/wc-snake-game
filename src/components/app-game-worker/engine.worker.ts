...
export class Game {
  ...
  constructor(
    ...
    private onGridChange: (grid: IPixelTypes[][]) => void,
  ...
  public restart() {
    ...
    this.grid.respawn();
    this.onGridChange(this.grid.drawGameFrame());
    ...
    const gameOverLoop = gameOverFrames => {
      ...
      try {
        if (gameOverFrames[gameOverFrameIndex]) {
          this.onGridChange(gameOverFrames[gameOverFrameIndex]);
          gameOverFrameIndex++;
        } else {
          clearTimeout(loopId);
        }
...
export async function init(
  size: IGameSizes,
  onGridChange: (grid: IPixelTypes[][]) => void,
